...
class _ModuleMeta(abc.ABCMeta):
    ...
    def __new__(mcs, name, bases, dict_, **kwargs):
        ...
        cls.__spekk_keys__ = lambda obj: [
            field.name for field in dataclasses.fields(obj)
        ]
        cls.__spekk_children__ = lambda obj: [
            getattr(obj, field.name) for field in dataclasses.fields(obj)
        ]
        cls.__spekk_recreate__ = lambda obj, keys, children: obj.__class__(
            **{k: v for k, v in zip(keys, children)}
        )
        return cls
...
def module_leaves(module: "Module") -> list:
    return [
        _field
        for _field in dataclasses.fields(module)
        if not _field.metadata.get("static", False)
    ]
